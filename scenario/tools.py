...
def makescen_altonly(model:Any) -> None:
    ...
    for f,y in z_sol:
        ...
        acid = idx2acid[f]
        # Get its route nodes
        rte_nodes = path_dict[acid]['paths'][0]
        # Get the departure time
        dep_time = tim2txt(path_dict[acid]['times'][0][0])
        print(path_dict[acid]['times'][0])
        ...
        for i, node in enumerate(rte_nodes):
            ...
            if i == len(rte_nodes)-1:
                ...
                next_lon = nodes.at[next_node,'geometry'].x
                next_lat = nodes.at[next_node,'geometry'].y
                # Get the angle
...
def makescen(model:Any, altonly:bool = False) -> None:
