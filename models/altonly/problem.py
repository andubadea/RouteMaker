...
class ProblemGlobal:
    ...
    def createVars(self) -> None:
        ...
        self.z = self.model.addVars(self.z_fy, 
                                    vtype = GRB.BINARY, 
                                    name = 'z')

    def createObjectiveFunction(self) -> None:
        self.model.setObjective(
            gb.quicksum(self.z[f,y] * (self.p.B_f[f] + self.p.Dlt_y[y])
                for f,y in self.z), gb.GRB.MINIMIZE) 

    def createConstraints(self) -> None:
        # First of all, one and only one altitude and route combination can be
        # allocated per flight.
        self.model.addConstrs((gb.quicksum(self.z[f,y] 
                                for y in self.p.Y 
                                ) == 1 
                            # forall vars
                            for f in self.p.F
