...
class Parameters:
    ...
    def compute_params(self) -> None:
        ...
        self.B_f, self.xp_fet, self.W_t, self.et_list \
    ...
    def process_paths(self, path_dict:dict) -> list:
        ...
        for acidx in self.F:
            ...
            for path_edge_idx, edge in enumerate(zip(path[:-1], path[1:])):
                ...
                for t in t_idx:
                    xp_fket[acidx,edge_idx,t] = 1
                # Set the maximum time window
                if t_idx_int > max_tw:
                    max_tw = t_idx_int
            
            #B_f_k wants flight times per path
            B_f.append(path_times[path_idx][-1]-path_times[path_idx][0])
